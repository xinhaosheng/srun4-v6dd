<template>
    <el-row class="panel">
        ...
        <el-row class="section search" :gutter="20">
            ...
            <el-col :span="8">
                <el-row type="flex" justify="end">
                    ...
                    <el-button type="primary" size="mini" icon="icon ion-md-search" @click="getData">搜索</el-button>
                </el-row>
            </el-col>
        </el-row>
        ...
        <!-- 高级搜索 -->
        <el-row
                class="section proSearch"
                :gutter="20"
                v-show="status.showPro"
                v-if="panelData.proSearchItem"
        >
            <el-col :span="6" v-for="(item,index) in panelData.data" :key="index">
                <el-input
            ...
            <el-form :model="editItem" label-position="left" label-width="120px">
                <el-form-item v-for="(item,index) in panelData.data" :key="index" :label="item.label">
                    ...
                    <el-row :gutter="10" v-if="editItemValSplice">
                        <el-col :span="6">
                            <el-select
                                    v-model="editItemValSplice[item.key].symbol"
                                    filterable
                                    clearable
                                    placeholder="选择前缀"
                            >
                        ...
                        <el-col :span="7">
                            <el-select
                                    v-model="editItemValSplice[item.key].ip"
                                    filterable
                                    clearable
                                    placeholder="请选择IP"
                            >
                        ...
                        <el-col :span="7">
                            <el-input v-model="editItemValSplice[item.key].subnetMask"  @keyup.enter.native="update"
                                      placeholder="请输入子网掩"
                                      clearable></el-input>
                        </el-col>
                    </el-row>
		...
		methods: {
			...
			editData(e) {
				for (let val of this.panelData.data) {
					...
					if (isSplice) {
						let temp =
							isSplice === "ipv4" ? rule_v4.exec(value) : rule_v6.exec(value);
						...
						if (subnetMaskGroup.length === 2)
							this.editItemValSplice[key].subnetMask = subnetMaskGroup[1];
			...
			update() {
				...
				if (this.editItemValSplice) {
					for (let key in this.editItemValSplice) {
						let str =
							this.editItemValSplice[key].symbol + this.editItemValSplice[key].ip;
						if (this.editItemValSplice[key].subnetMask)
							str += `/${this.editItemValSplice[key].subnetMask}`;
